...
/**
 * LIVE location suggestions (Manage Bookings)
 * - Only City / State / ZIP results (no hotels, no POIs, no county-only)
 * - ZIP required (must be present either in suggestion or typed)
 */
...
function buildSubtitle(address, postal) {
...
function pickPrimaryName(item) {
  const a = item.address || {};
  // For Manage Bookings: prefer city-level name
  const city = a.city || a.town || a.village || "";
  return city || (item.display_name || "").split(",")[0]?.trim() || "City";
}
...
/**
 * Dummy reservations (MVP)
 * Added zipCode for area filtering.
 */
const DUMMY_BOOKINGS = [
  ...
  },
  {
    ...
    costBreakdown: { base: 330, fees: 30, total: 360 },
  },
];
        ...
        <Stack spacing={2} sx={{ pt: 1 }}>
            ...
            <Box>
              ...
              <Stack direction="row" spacing={1} mt={1} flexWrap="wrap">
                ...
                <Chip
                  ...
                  size="small"
                  color={paymentChipColor(booking.paymentStatus)}
                />
              </Stack>
          ...
          <Box>
            ...
            <Typography variant="body2">
              ...
              {new Date(booking.returnDate).toLocaleDateString()} ({duration} days)
            </Typography>
          </Box>
          ...
          <Box>
            ...
            <Stack direction="row" justifyContent="space-between">
              ...
              <Typography fontWeight={900}>${booking.costBreakdown.total.toFixed(2)}</Typography>
            </Stack>
          </Box>
          ...
          <Box>
            ...
            <Typography variant="body2" color="text.secondary">
              {(booking.insurance || []).length ? booking.insurance.join(", ") : "Standard coverage"}
            </Typography>
          </Box>
          ...
          <Box>
            ...
            <Typography variant="body2" color="text.secondary">
              Invoice PDF coming soon. (MVP placeholder)
            </Typography>
          </Box>
          ...
          <Box>
            ...
            <Typography variant="body2" color="text.secondary">
              QR coming soon. (MVP placeholder)
            </Typography>
          </Box>
        </Stack>
      </DialogContent>
...
export default function ManageBookings() {
  ...
  // Location (single field: City/State/ZIP)
  ...
  // Results filtered by area after selection
  ...
  // ZIP requirement (inside same field)
  const selectedZip = useMemo(() => {
    return locationValue?.zip || extractZipFromText(locationInput);
  ...
  // Live location search (MANAGE BOOKINGS = only city-level)
  useEffect(() => {
...
// -------------------------
  ...
  // Auto-filter results by AREA as soon as a location is selected (even before Find)
  useEffect(() => {
    ...
    const areaFiltered = DUMMY_BOOKINGS.filter((b) => {
      const matchesZip = b.zipCode === zip;
      const matchesCity = b.pickupLocation.toLowerCase().includes(city);
  ...
  const handleSearch = () => {
    ...
    if (!canSearch) {
      setMsg("ZIP is required. Pick a City/State/ZIP suggestion (no hotels/POIs).");
      return;
    }
    ...
    const found = DUMMY_BOOKINGS.filter((b) => {
      const matchesLn = b.lastName.toLowerCase() === ln;
      const matchesRn = b.reservationNumber.toUpperCase() === rn;
      const matchesZip = b.zipCode === zip;

      // city soft match
      const matchesCity = b.pickupLocation.toLowerCase().includes(selectedCity);

      return matchesLn && matchesRn && matchesZip && matchesCity;
    });

    if (!found.length) {
      setMsg("No reservation found for that Last Name + Reservation + Area ZIP (MVP dummy data).");
      return;
    }
  ...
  const cancelReservation = (booking) => {
    if (booking.status !== "UPCOMING") {
      setMsg("Only UPCOMING reservations can be cancelled (MVP).");
      return;
    }
    ...
    const updated = results.map((r) =>
      r.id === booking.id ? { ...r, status: "CANCELLED" } : r
    );
    setResults(updated);
  ...
  return (
    <Box sx={{ flexGrow: 1, bgcolor: "#f5f7fa", minHeight: "100vh" }}>
      ...
      >
        <Container maxWidth="lg">
          <Toolbar disableGutters sx={{ justifyContent: "space-between" }}>
            <Typography
              variant="h6"
              sx={{ fontWeight: 800, color: "#1A237E", cursor: "pointer" }}
              onClick={() => navigate("/")}
            >
      ...
      <Box sx={{ bgcolor: "#1A237E", py: isMobile ? 5 : 7, color: "white" }}>
        <Container maxWidth="lg">
          ...
          <Typography
            variant="body1"
            textAlign="center"
            sx={{ opacity: 0.9, maxWidth: 820, mx: "auto" }}
          >
          ...
          <Box sx={{ p: isMobile ? 2.5 : 4 }}>
            <Stack spacing={2.5}>
              <Box>
                ...
                <Typography variant="body2" color="text.secondary">
                  Manage Bookings shows only City-level results (with ZIP required).
                </Typography>
              </Box>

              {msg && (
                <Alert
                  severity={msg.includes("Found") ? "success" : msg.includes("cancelled") ? "info" : "warning"}
                  onClose={() => setMsg(null)}
                >
                  {msg}
                </Alert>
              ...
              <Autocomplete
                ...
                noOptionsText={
                  debouncedLocationInput.trim().length < 2
                    ? "Type City, State, or ZIP (e.g., 77001)"
                    : "No city-level locations found (try ZIP)"
                }
                renderInput={(params) => (
                  <TextField
                    {...params}
              ...
              {locationValue && !/^\d{5}$/.test(selectedZip) && (
                <Alert severity="warning">
                  ZIP is required. Please pick a suggestion that includes a ZIP (or type a ZIP).
                </Alert>
              )}
              ...
              >
                ...
                <Button variant="text" sx={{ textTransform: "none" }} onClick={handleClear}>
                  Clear
                </Button>
              </Stack>
              ...
              <Box sx={{ pt: 0.5 }}>
                <Typography variant="body2" sx={{ color: "text.secondary" }}>
                  ...
                  <Typography
                    ...
                    to="/login"
                    variant="body2"
                    sx={{ fontWeight: 900, color: "#1A237E", textDecoration: "none" }}
                  >
                    Sign In.
                  </Typography>
                </Typography>
                ...
                <Typography variant="body2" sx={{ color: "text.secondary", mt: 0.5 }}>
                  To view all reservations associated with your <b>DRIVEFLOW</b> account,
                  please{" "}
                  <Typography
                    component={Link}
                    to="/login"
              ...
              <Stack direction="row" spacing={1} flexWrap="wrap">
                <Chip label="City-level only (no hotels/POIs)" size="small" variant="outlined" />
                <Chip label="ZIP required in same field" size="small" variant="outlined" />
                <Chip label="Agreement PDF download (MVP)" size="small" variant="outlined" />
              </Stack>
            </Stack>
          </Box>
        </Paper>
      ...
      {/* RESULTS (already filtered by area once location selected) */}
      <Container maxWidth="lg" sx={{ pb: 6 }}>
        {locationValue && /^\d{5}$/.test(selectedZip) && (
          <Typography variant="body2" sx={{ color: "text.secondary", mb: 2 }}>
            Area selected: <b>{locationValue.primary}</b> • ZIP <b>{selectedZip}</b>
          </Typography>
        )}
          ...
          >
            <Box sx={{ p: isMobile ? 2 : 3 }}>
              ...
              <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                Auto-filtered by City + ZIP (dummy data).
              </Typography>
                    ...
                    {results.map((b) => (
                      <TableRow key={b.id} hover>
                        <TableCell>
                          <Stack direction="row" spacing={2} alignItems="center">
                            <Box
                              ...
                              src={b.car.imageUrl}
                              alt={b.car.makeModel}
                              sx={{ width: 80, height: 52, borderRadius: 2, objectFit: "cover" }}
                            />
                            <Box>
                        ...
                        <TableCell align="right">
                          <IconButton title="View Details" onClick={() => openDetails(b)}>
                            <Visibility />
                          </IconButton>
                          <IconButton title="Cancel" onClick={() => cancelReservation(b)} disabled={b.status !== "UPCOMING"}>
                            <Cancel />
                          </IconButton>
                          <IconButton title="Rebook" onClick={() => rebook(b)}>
                            <Replay />
                          </IconButton>
                          <IconButton title="Download Agreement (PDF)" onClick={() => downloadAgreement(b)}>
                            <Description />
                          </IconButton>
                        </TableCell>
                      </TableRow>
                  ...
                  {results.map((b) => (
                    <Card key={b.id} variant="outlined" sx={{ borderRadius: 4 }}>
                        ...
                        <Stack spacing={1.5}>
                          <Box display="flex" gap={2}>
                            <Box
                              ...
                              src={b.car.imageUrl}
                              alt={b.car.makeModel}
                              sx={{ width: 120, height: 84, borderRadius: 2, objectFit: "cover" }}
                            />
                            <Box flex={1}>
                              <Typography fontWeight={900}>{b.car.makeModel}</Typography>
                          ...
                          <Typography variant="body2">
                            <b>Return:</b> {b.returnLocation}
                          </Typography>
                          <Typography variant="body2">
                            <b>Total:</b> ${Number(b.totalAmount).toFixed(2)}
                          </Typography>
                        </Stack>
                      </CardContent>
                      ...
                      <CardActions sx={{ px: 2, pb: 2, gap: 1, flexWrap: "wrap" }}>
                        ...
                        <Button size="small" variant="outlined" onClick={() => rebook(b)} startIcon={<Replay />}>
                          Rebook
                        </Button>
        ...
        {locationValue && /^\d{5}$/.test(selectedZip) && results.length === 0 && (
          <Alert severity="info">
            No dummy reservations match this City + ZIP yet. (MVP)
          </Alert>
        )}
      </Container>
